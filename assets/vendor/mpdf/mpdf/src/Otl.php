		...
		for ($sch = 0; $sch <= $subchunk; $sch++) {
			...
			if (($useOTL & 0xFF) && $GSUBscriptTag && $GSUBlangsys && $GSUBFeatures) {
				...
				//==============================
					 ...
					 * to be what Unimcribe and other engines implement.  According to Eric Muller:
					 *
					 ...
					 *
					 * Testing shows that Unimcribe reorder the following marks:
			...
			foreach ($LookupList as $lu => $tag) {
				...
				while ($ptr < (count($this->OTLdata))) { // whilst there is another glyph ..0064
					...
					foreach ($this->GSUBLookups[$lu]['Subtables'] as $c => $subtable_offset) {
						...
						// incorrectly just moved old spec tables to new spec. Unimcribe seems to cope with this
		...
		elseif ($Type == 4) {
			...
			if ($contains_marks && !$contains_nonmarks) {
				...
				for ($i = 1; $i < count($GlyphPos); $i++) {
					// If subsequent components are not Marks as well - don't ligate
					//      $unimtr = $this->OTLdata[$GlyphPos[$i]]['hex'];
					//      if (strpos($this->GlyphClassMarks, $unimtr )===false) { return; }
...
//echo "PROBLEM: ".($x).'<br />';
	 ...
	 * Secondly re-order - called from fn writeFlowingBlock and FinimhFlowingBlock, when already divided into lines. [_bidiReorder]
	 */
