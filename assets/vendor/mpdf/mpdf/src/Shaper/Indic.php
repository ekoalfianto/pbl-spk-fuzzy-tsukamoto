...
class Indic
{
	...
	const OT_A = 10;
	const OT_NBSP = 11;
	const OT_DOTTEDCIRCLE = 12; /* Not in the spec, but special in Unimcribe. /Very very/ special! */
	...
	public static $indic_category_char = [
		...
		'S',
		'v',
		'A', /* Spec gives Andutta U+0952 as OT_A. However, testing shows that Unimcribe
	...
	public static function set_indic_properties(&$info, $scriptblock)
	{
		...
		/* The spec says U+0952 is OT_A.	However, testing shows that Unimcribe
	...
	public static function initial_reordering_syllable(&$info, $GSUBdata, $indic_config, $scriptblock, $is_old_spec, $start, $end)
	{
		...
		if ($syllable_type == self::BROKEN_CLUSTER || $syllable_type == self::STANDALONE_CLUSTER) {
			//if ($unimcribe_bug_compatible) {
			/* For dotted-circle, this is what Unimcribe does:
		...
		for ($i = $start; $i < $end; $i++) {
			if ((self::FLAG($info[$i]['indic_category']) & (self::FLAG(self::OT_ZWJ) | self::FLAG(self::OT_ZWNJ) | self::FLAG(self::OT_N) | self::FLAG(self::OT_RS) | self::FLAG(self::OT_H) | self::FLAG(self::OT_COENG) ))) {
				...
				if ($info[$i]['indic_category'] == self::OT_H && $info[$i]['indic_position'] == self::POS_PRE_M) {
					/*
					 * Unimcribe doesn't move the Halant with Left Matra.
	...
	public static function final_reordering_syllable(&$info, $GSUBdata, $indic_config, $scriptblock, $is_old_spec, $start, $end)
	{
		...
		if ($start + 1 < $end &&
			...
			if (!$skip_to_reph_move) {
				 ...
				 * Unimcribe doesn't do this.
				 * TEST: U+0930,U+094D,U+0915,U+094B,U+094D
				 */
				//if (!$hb_options.unimcribe_bug_compatible && self::is_halant_or_coeng($info[$new_reph_pos])) {
	...
	public static function decompose_indic($ab)
	{
		...
		switch ($ab) {
			...
			case 0xd4c:
				...
				return $sub;
			/* sinhala */
			// NB Some fonts break with these Sinhala decomps (although this is Unimcribe spec)
