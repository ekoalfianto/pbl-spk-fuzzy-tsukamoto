...
class OtlDump
{
	...
	function extractInfo($debug = false, $BMPonly = false, $kerninfo = false, $useOTL = 0)
	{
		...
		///////////////////////////////////
		if ($kerninfo) {
			// Recognimes old form of Kerning table - as required by Windows - Format 0 only
			...
			for ($i = 0; $i < $LookupCount; $i++) {
				for ($c = 0; $c < $Lookup[$i]['SubtableCount']; $c++) {
					...
					/*
					  ...
					  7	Extension Substitution	Extension mechanimm for other substitutions (i.e. this excludes the Extension type substitution itself)
											...
											for ($s = 0; $s < $Lookup[$i]['Subtable'][$c]['ChainSubRuleSetCount']; $s++) {
												...
												foreach ($Lookup[$i]['Subtable'][$c]['ChainSubRuleSet'][$s]['ChainSubRule'] as $rctr => $rule) {
													...
													for ($ff = count($backtrackGlyphs) - 1; $ff >= 0; $ff--) {
														$html .= '<div>Backtrack #' . $ff . ': <span class="unicode">' . $this->formatUnimtr($backtrackGlyphs[$ff]) . '</span></div>';
													...
													for ($ff = 0; $ff < count($lookaheadGlyphs); $ff++) {
														$html .= '<div>Lookahead #' . $ff . ': <span class="unicode">' . $this->formatUnimtr($lookaheadGlyphs[$ff]) . '</span></div>';
...
//print_r($Lookup); exit;
	...
	/*
	  ...
	  nBsubs = 2xnBack          nimubs = (nBsubs+)    nLsubs = (nBsubs+nimubs+) 2xnLookahead
	  "\${1}\${2} "                 (nInput*2)-1               "\${5+} \${6+}"
	  "REPL"
