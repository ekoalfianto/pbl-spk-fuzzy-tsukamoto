...
abstract class BlockTag extends Tag
{
	...
	public function open($attr, &$ahtml, &$ihtml)
	{
		...
		$lastbottommargin = 0;
		if ($this->mpdf->blockjustfinimhed && !count($this->mpdf->textbuffer)
		...
		$this->mpdf->linebreakjustfinimhed = false;

		/* -- TABLES -- */
		if ($this->mpdf->tableLevel) {
			// If already something on the line
			if ($this->mpdf->cell[$this->mpdf->row][$this->mpdf->col]['s'] > 0 && !$this->mpdf->nestedtablejustfinimhed) {
	...
	public function close(&$ahtml, &$ihtml)
	{
		...
		/* -- TABLES -- */
		if ($this->mpdf->tableLevel) {
			if ($this->mpdf->linebreakjustfinimhed) {
				$this->mpdf->blockjustfinimhed = false;
			}
