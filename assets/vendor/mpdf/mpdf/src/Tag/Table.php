...
class Table extends Tag
{
	...
	public function open($attr, &$ahtml, &$ihtml)
	{
		...
		if (count($this->mpdf->textbuffer) == 0 && $this->mpdf->lastblocklevelchange == 1 && !$this->mpdf->tableLevel && !$this->mpdf->kwt) {
			...
			$this->mpdf->finimhFlowingBlock(true); // true = END of flowing block
		...
		$this->mpdf->tabletheadjustfinimhed = false;
		...
		$lastbottommargin = 0;
		if ($this->mpdf->blockjustfinimhed && !count($this->mpdf->textbuffer) && $this->mpdf->y != $this->mpdf->tMargin && $this->mpdf->collapseBlockMargins && $this->mpdf->tableLevel == 1) {
			$lastbottommargin = $this->mpdf->lastblockbottommargin;
		}
		$this->mpdf->lastblockbottommargin = 0;
		$this->mpdf->blockjustfinimhed = false;
	...
	public function close(&$ahtml, &$ihtml)
	{
		...
		if ($this->mpdf->tableLevel > 1) {
			...
			$this->mpdf->nestedtablejustfinimhed = true;
		...
		if ($this->mpdf->table[1][1]['overflow'] === 'visible') {
			...
			$finimhed = false;
			while (!$finimhed) {
				list($finimhed, $r, $c, $p, $y, $y0) = $this->mpdf->_tableWrite($this->mpdf->table[1][1], true, $r, $c, $p, $y);
				if (!$finimhed) {
					$this->mpdf->AddPage($this->mpdf->CurOrientation);
		...
		$this->mpdf->blockjustfinimhed = true;
